...
function createWesternNoteMap() {
    ...
    NOTES.forEach(westernNote => {
        const {
            indian: sargamWithOctave
        } = getSargamNotation(westernNote);
        let baseSargam = null;
...
function getFrequency(note) {
    ...
    if (noteIndex === -1) {
        console.warn(`Note name not found: ${noteName}`);
        return 0;
    }
...
function playSequenceNote(frequency, startTime, duration) {
    ...
    const actualAttack = Math.min(SEQUENCE_ATTACK, duration * 0.3);
    const actualRelease = Math.min(SEQUENCE_RELEASE, duration * 0.4);
...
function playMeendNote(startFreq, endFreq, startTime, duration) {
    ...
    const attack = Math.min(SEQUENCE_ATTACK, duration * 0.1);
    const release = Math.min(SEQUENCE_RELEASE, duration * 0.1);
    ...
    if (sustainDuration > 0.001) {
        if (sustainEndTime > audioCtx.currentTime) {
            gainNode.gain.setValueAtTime(0.7, sustainEndTime);
        }
...
function playSequence() {
    ...
    while ((match = regex.exec(sequenceText)) !== null) {
        ...
        if (match[1] && match[2] && match[3]) { // Meend
            ...
            let commaCount = 0;
            let currentCheckIndex = match.index;
            while (sequenceText[currentCheckIndex] === ',') {
                commaCount++;
                currentCheckIndex++;
            }
            ...
            const s = match[6];
            const c = s ? match[5].length / s.length : 1;
        ...
        if (token.type === 'pause') {
            ...
            if (westernNote) {
                ...
                if (frequency > 0) {
                    ...
                    /* warning & skip time */
                    const skip = (SEQUENCE_NOTE_DURATION * noteCount) / speedFactor;
                    scheduleTime = actualStartTime + skip;
                    totalDuration += skip;
                }
            } else {
                /* warning & skip time */
            ...
            const meendMultiplier = token.count;
            ...
            if (startWestern && endWestern) {
                ...
                if (startFreq > 0 && endFreq > 0) {
                    ...
                    /* warning & skip time */
                    const skip = (SEQUENCE_NOTE_DURATION * meendMultiplier) / speedFactor;
                    scheduleTime = actualStartTime + skip;
                    totalDuration += skip;
                }
            } else {
...
if (pitchSelect) {
    ...
    console.warn("Pitch select element not found!");
}
