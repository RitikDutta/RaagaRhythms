crepe = (function() {
  ...
  function updatePitchDisplay(hz, confidence) {
    ...
    const hzEl = document.getElementById('pitch-hz');
    const centsEl = document.getElementById('pitch-cents');
    const confFill = document.getElementById('confidence-fill');

    if (confFill) {
      confFill.style.width = clamp(confidence * 100, 0, 100).toFixed(1) + '%';
    }

    if (!noteEl || !hzEl || !centsEl) {
      return;
    }

    if (!isFinite(hz) || hz <= 0 || confidence <= 0.5) {
      noteEl.textContent = '--';
      hzEl.textContent = '-- Hz';
      centsEl.textContent = 'No voice';
      return;
    }
  ...
  const updatePitchGraph = (function() {
    ...
    function drawGrid() {
      ...
      gridLabels.forEach(function(row) {
    ...
    return function(hz, confidence) {
      ...
      ctx.save();
      ctx.shadowColor = 'rgba(247, 183, 51, 0.35)';
      ctx.shadowBlur = 14;
      ctx.strokeStyle = 'rgba(247, 183, 51, 0.35)';
      ctx.lineWidth = 6;
      buildPath();
      ctx.stroke();
      ctx.restore();
      ...
      ctx.save();
      ctx.strokeStyle = 'rgba(255, 209, 128, 0.95)';
      ctx.lineWidth = 2;
      buildPath();
      ctx.stroke();
      ctx.restore();
  ...
  try {
    ...
    document.querySelectorAll('[data-srate]').forEach(function(node) {
      node.textContent = audioContext.sampleRate;
    });
  } catch (e) {
  ...
  function process_microphone_buffer(event) {
    resample(event.inputBuffer, function(resampled) {
      tf.tidy(() => {
        ...
        document.getElementById('voicing-confidence').textContent = confidence.toFixed(3);
        ...
        updatePitchDisplay(predicted_hz, confidence);
        updatePitchGraph(predicted_hz, confidence);
